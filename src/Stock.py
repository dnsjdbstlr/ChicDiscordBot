...
async def 주식(ctx):
    ...
    for i in holding.values():
        ...
        Tool.updateAuctionPrice(i['name'])

    # 계산
    totBid = getTotBid(holding)
    totGain = getTotGain(holding)
    totProfit = getTotProfit(totBid, totGain)

    # 결과
    embed = discord.Embed(title=ctx.message.author.display_name + '님의 주식 정보를 알려드릴게요.')
    embed.add_field(name='> 보유 금액', value=f"{format(Tool.getGold(did), ',')}골드", inline=False)
    embed.add_field(name='> 총 수익률', value=totProfit)
    embed.add_field(name='> 총 매수금', value=f"{format(totBid, ',')}골드")
    embed.add_field(name='> 총 평가금', value=f"{format(totGain, ',')}골드")

    for index, h in enumerate(holding.values()):
        try:
            name = f"> {h['name']}"
            value =  f"현재 단가 : {format(Tool.getTodayPrice(h['name']), ',')}골드\r\n"
            value += f"매수 단가 : {format(h['bid'], ',')}골드\r\n"
            value += f"매수량 : {format(h['count'], ',')}개\r\n"
            value += f"수익률 : {Util.getVolatility(h['bid'], Tool.getTodayPrice(h['name']))}"
        except:
            name = f'> 종목 {index + 1}'
...
async def 매수(bot, ctx, *input):
    await ctx.message.delete()
    if not input:
        await ctx.channel.send("> !매수 '아이템이름' 의 형태로 적어야해요!")
        return
    ...
    embed.add_field(name='> 단가',        value=format(latest['price'], ',') + '골드')
    ...
    try:
        ...
        if str(reaction) == '⭕':
            await msg.delete()
            await buyStock(bot, ctx, name, latest['price'])
        elif str(reaction) == '❌':
            await msg.delete()
...
async def 매도(bot, ctx):
    ...
    if getHoldingCount(holding) == 0:
        ...
        text += "> `!매수 '아이템이름'` 명령어를 사용해서 주식을 매수해보세요!"
        await waiting.delete()
        await ctx.channel.send(text)
        return
    ...
    for i in holding.values():
        if i is None: break
        Tool.updateAuctionPrice(i['name'])
        value =  f"매도 단가 : {format(Tool.getLatestPrice(i['name'])['price'], ',')}\r\n"
        value += f"매수 단가 : {format(i['bid'], ',')}\r\n"
        value += f"보유량 : {format(i['count'], ',')}"
...
async def 주식랭킹(bot, ctx):
    ...
    rank    = getSortedStockRank(stocks)
...
async def buyStock(bot, ctx, name, price):
    ...
    itemId = DNFAPI.getMostSimilarItem(name)['itemId']
    ...
    embed = discord.Embed(title=ctx.message.author.display_name + '님의 매수 주문',
                          description='매수 수량을 입력해주세요.')
    embed.add_field(name='> 보유 금액', value=format(gold, ',') + '골드')
    embed.add_field(name='> 매수 단가', value=format(price, ',') + '골드')
    embed.add_field(name='> 매수 가능 수량', value=format(gold // price, ',') + '개')
    embed.set_footer(text='30초 안에 결정해야합니다.')
    embed.set_thumbnail(url=DNFAPI.getItemImageUrl(itemId))
    msg = await ctx.channel.send(embed=embed)
    ...
    try:
        ...
        if isOverride:
            for i in holding.values():
                if i['name'] == name:
                    ...
                    newBid   = ((price * count) + (i['bid'] * i['count'])) // newCount
        ...
        embed2.add_field(name='> 매수 단가', value=format(price, ',') + '골드')
        embed2.add_field(name='> 매수량',    value=format(count, ',') + '개')
        embed2.add_field(name='> 매수금',    value=format(count * price, ',') + '골드')
        embed.set_thumbnail(url=DNFAPI.getItemImageUrl(itemId))
        await msg.edit(embed=embed2)
    except asyncio.TimeoutError:
        await msg.delete()
...
async def sellStock(bot, ctx, holding, index):
    did    = ctx.message.author.id
    offer  = Tool.getLatestPrice(holding[str(index)]['name'])['price']
    itemId = DNFAPI.getMostSimilarItem(holding[str(index)]['name'])['itemId']

    embed  = discord.Embed(title=ctx.message.author.display_name + '님의 매도 주문',
                           description='매도 수량을 입력해주세요.\r\n매도 채결 시 1%의 수수료가 발생해요.')
    embed.add_field(name='> 종목', value=holding[str(index)]['name'])
    embed.add_field(name='> 매도 단가', value=format(offer, ',') + '골드')
    embed.add_field(name='> 매도 가능 수량', value=format(holding[str(index)]['count'], ',') + '개')
    embed.set_footer(text='30초 안에 결정해야합니다.')
    embed.set_thumbnail(url=DNFAPI.getItemImageUrl(itemId))
    msg = await ctx.channel.send(embed=embed)
...
def getSortedStockRank(stocks):
    def key(x):
        value = Tool.getGold(x['did'])
        for i in json.loads(x['holding']).values():
            try:
                value += Tool.getLatestPrice(i['name'])['price'] * i['count']
            except: continue
        return value
...
def getTotGain(holding):
    totGain = 0
    for index, h in enumerate(holding.values()):
        try:
            totGain += Tool.getLatestPrice(h['name'])['price'] * h['count']
        except: continue
    return totGain
...
def getTotProfit(totBid, totGain):
    try:
        ...
        return setFluctuationMark(totProfit)
    except:
        return '- 0.00%'

def setFluctuationMark(num):
    if num >= 0:
        num = '▲ ' + format(num, ',') + '%'
    elif num == 0:
        num = '- 0.00%'
