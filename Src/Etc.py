...
def getChicBotDesc(page):
    if page == 1:
        ...
        title = "시크봇의 '선물거래' 관련 명령어들을 알려드릴게요!"
    elif page == 3:
    ...
    if page == 1:
        ...
        embed.add_field(name="> !에픽 '닉네임'",         value='해당 캐릭터가 이번 달에 획득한 에픽을 알려드릴게요.')
        embed.add_field(name='> !에픽랭킹',              value='이번 달 획득한 에픽 개수를 기준으로한 랭킹을 보여드려요.')
    elif page == 2:
        embed.add_field(name='> !선물거래',               value='던파 경매장을 기반으로한 선물 거래에 대한 설명을 알려드려요.')
        embed.add_field(name="> !주문 '종목' '레버리지'", value='해당 포지션의 주문을 넣어요.')
        embed.add_field(name="> !포지션",                 value='보유 중인 포지션을 확인하고 종료할 수 있어요.')
        embed.add_field(name="> !거래내역",               value='최근 6번의 선물 거래 내역을 보여드려요.')
        embed.add_field(name="> !파산",                   value='파산을 신청해요. 골드와 포지션이 모두 사라지고 3일동안 선물 거래를 할 수 없어요.')
        embed.add_field(name="> !골드랭킹",               value='보유금과 평가금을 합한 것을 기준으로한 랭킹을 보여드려요.')
    elif page == 3:
        ...
        embed.add_field(name="> !출석",               value='매일마다 골드를 얻을 수 있어요. 추후 더 많은 효과가 추가될 수 있어요!')
